...
let explore ~exploration_timeout ~max_programs ~program_size_limit ~eval_timeout
    ~attempts ~dsl ~representations_dir j =
  let max_conn = SU.to_int @@ SU.member "max_conn" j in
  let retrieve_result () = Util.value_exn !Operations.last_found in
  Exploration.multikey_explore ~exploration_timeout ~max_programs
